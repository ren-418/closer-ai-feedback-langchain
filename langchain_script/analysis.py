...
def build_chunk_analysis_prompt(chunk_text: str, reference_texts: List[Dict], context_prev: str = '', context_next: str = '', business_rules: List[Dict] = None) -> str:
    # STRICT STYLE & CONTENT REQUIREMENTS (REAL-WORLD, CONCRETE)
    style_requirements = (
        "STRICT STYLE & CONTENT REQUIREMENTS (APPLY TO ALL SECTIONS):\n"
        "- DO NOT mention or refer to 'the closer', 'the lead', or any subject person in any section, including reference comparison sections (similarities, differences, best practices, missed opportunities). All feedback must be written in a way that focuses only on actions, content, techniques, or outcomes, without referencing the subject person.\n"
        "- DO NOT use negative or boilerplate statements such as: 'No objections were raised in this chunk/segment/section.', 'No explicit closing attempts were made in this chunk.', 'No X in this chunk/segment/section.'\n"
        "- DO NOT repeat the same phrase or bullet point within or across sections. If a point is similar, group and summarize it.\n"
        "- Each section should contain only the most important, non-redundant points (maximum 3-5 per section).\n"
        "- If a section has no substantive content, OMIT IT ENTIRELY. Do not include empty sections or sections with only negative/boilerplate statements.\n"
        "- Write in a professional, concise, and aggregation-ready style. Avoid filler, hedging, or generic language.\n"
        "- Group similar moments or actions into a single, concise bullet. Do not list the same event in multiple ways.\n"
        "- DO NOT copy the same phrase or sentence into multiple sections. Each point should be unique and relevant to its section.\n"
        "\n"
        "BAD EXAMPLES (Do NOT do this):\n"
        "- 'No objections were raised in this chunk.'\n"
        "- 'No explicit closing attempts were made in this chunk.'\n"
        "- 'Built rapport by engaging in casual conversation about their locations and weather conditions.' (repeated 5+ times)\n"
        "- 'The closer missed an opportunity to address the lead’s concern about tracking income.' (repeated in multiple sections)\n"
        "- 'Maintained a casual and friendly tone throughout the conversation.' (repeated or generic)\n"
        "- 'The closer did not address any objections as none were raised during this portion of the call.'\n"
        "- 'No objections were raised in this segment.'\n"
        "- 'Engaged in casual conversation to build rapport.' (repetitive, generic)\n"
        "- 'The closer effectively built rapport by empathizing with the lead’s situation.' (mentions subject person)\n"
        "- 'This mirrors successful approaches from top performers who use casual conversation to build rapport at the beginning of the call.' (reference comparison mentions subject person)\n"
        "- 'In successful examples, the closer would address this concern by offering solutions or strategies for effective product sourcing.' (reference comparison mentions subject person)\n"
        "- 'This is similar to successful examples where the closer asks about the lead’s previous business attempts to uncover challenges and tailor the solution.' (reference comparison mentions subject person)\n"
        "\n"
        "GOOD EXAMPLES (Do this):\n"
        "- Rapport Building (Strengths):\n"
        "  'Rapport was built through casual conversation about locations, weather, and personal stories.'\n"
        "  'Empathy shown by acknowledging goals and sharing relevant personal experiences.'\n"
        "- Rapport Building (Weaknesses):\n"
        "  'Missed opportunity to explore interests in snorkeling and personal hobbies.'\n"
        "- Objection Handling (Strengths):\n"
        "  'Concerns about changing jobs were addressed by sharing personal experience and offering reassurance.'\n"
        "  'Payment objections handled by suggesting alternative solutions and maintaining a positive tone.'\n"
        "- Objection Handling (Weaknesses):\n"
        "  'Did not address concerns about tracking income or product sourcing.'\n"
        "  'Missed opportunity to proactively address potential objections about credit card debt.'\n"
        "- Closing Effectiveness (Strengths):\n"
        "  'Conversation guided toward commitment by discussing financial goals and program benefits.'\n"
        "  'Payment options explained clearly and next steps encouraged.'\n"
        "- Closing Effectiveness (Weaknesses):\n"
        "  'Failed to provide clear instructions during the payment process.'\n"
        "- Discovery/Qualification (Strengths):\n"
        "  'Probing questions asked about financial goals, previous Amazon experience, and payment preferences.'\n"
        "- Discovery/Qualification (Weaknesses):\n"
        "  'Not enough follow-up questions to understand financial situation and business history.'\n"
        "- Reference Comparisons (Similarities, Differences, Best Practices, Missed Opportunities):\n"
        "  BAD: 'This mirrors successful approaches from top performers who use casual conversation to build rapport at the beginning of the call.'\n"
        "  BAD: 'In successful examples, the closer would address this concern by offering solutions or strategies for effective product sourcing.'\n"
        "  GOOD: 'Rapport was built at the start of the call through discussion of shared interests, as seen in top-performing calls.'\n"
        "  GOOD: 'Missed opportunity to address product sourcing concerns; top calls provide specific strategies such as leveraging Amazon’s marketplace or using product research tools.'\n"
        "  GOOD: 'Probing questions about business history and challenges are a common feature in high-performing calls.'\n"
        "\n"
        "FINAL REMINDER:\n"
        "- Do NOT include any negative, empty, or boilerplate statements.\n"
        "- Do NOT repeat points or copy the same phrase into multiple sections.\n"
        "- Omit any section that would otherwise be empty or generic.\n"
        "- Focus on actionable, specific, and non-redundant feedback only.\n"
        "- If you are unsure whether to include a point, leave it out. It is better to omit than to include generic, negative, or repetitive content.\n"
        "\n"
    )
